...
class ResNetBasicHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for pathway in range(self.num_pathways):
            ...
            # print("pool_out", m(inputs[pathway]).shape)
        x = torch.cat(pool_out, 1)
        # (N, C, T, H, W) -> (N, T, H, W, C).
        ...
        # if hasattr(self, "dropout"):
        #     x = self.dropout(x)
        # # print(f"in head before projection {x.shape}")
        # if isinstance(self.num_classes, (list, tuple)):
        #     # print(f"in head before projection {x[0].shape}")
        #     x_v = self.projection_verb(x)
        #     x_n = self.projection_noun(x)
        #     # print(f"in head after projection {x_n.shape}")
